...
25) comparing whether two graphs are the same;
...
5) evaluating the smallest number of holes on the surface, on which the graph can be drawn without any of its links intersecting;
...
1) the method for adding a new node accepts an iter, containing nodes, that point to the new node and an iter, containing nodes, that the new node points to;
...
3) the degrees method returns a pair of numbers, the first of which shows how many nodes point to it and the second one shows how many nodes it points to, if a node is given,
